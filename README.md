...
Input nodes in a computational graph can be defined using ad.Variable. For instance, the input variable nodes  and  might be set up as follows:
...
, with x1 and x2 already established as input variables, the rest of the graph can be defined using just one line of Python:
...
Returning to our earlier example, once you have implemented the gradients function, you can use it to calculate the gradients of  with respect to  and . This is done by running:
...
Implement a linear transformation module that performs: 
...
Forward Pass:
x is shape [1, 5] and is expanded to shape [3, 5]. It creates a view that repeats the single row across the new dimension until the shape matches [3, 5].
